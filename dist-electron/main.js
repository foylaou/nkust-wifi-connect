import { app, nativeImage, Tray, ipcMain, Menu, BrowserWindow } from "electron";
...
var sudoPrompt = {};
var hasRequiredSudoPrompt;
function requireSudoPrompt() {
  if (hasRequiredSudoPrompt) return sudoPrompt;
  hasRequiredSudoPrompt = 1;
  var Node = {
    child: require$$0$1,
    crypto: require$$1$1,
    fs: require$$2,
  ...
  function LinuxBinary(instance, end) {
    ...
    function test2() {
      ...
      Node.fs.stat(
        ...
        function(error) {
          if (error) {
            if (error.code === "ENOTDIR") return test2();
            if (error.code === "ENOENT") return test2();
            end(error);
          } else {
            end(void 0, path2);
          }
...
var sudoPromptExports = requireSudoPrompt();
function getSsidWithIpconfig() {
...
function runVerboseSudo() {
  ...
  return new Promise((resolve, reject) => {
    sudoPromptExports.exec(command, options, (error, _stdout, stderr) => {
      if (error) {
...
async function loginNKUST(username, password) {
  try {
    ...
    if ([302, 303].includes(loginResp.status)) {
      const test2 = await axios.get("http://neverssl.com", { timeout: 3e3, validateStatus: () => true });
      if (test2.status === 200) return "登入成功 ✅";
    ...
    const error = err;
    const errorMsg = (error == null ? void 0 : error.message) || String(err);
...
app.whenReady().then(() => {
  const iconPath = path.join(__dirname$1, "../assets/tray-icon.png");
